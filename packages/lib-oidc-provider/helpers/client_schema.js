...
function parseRedirectUri(redirectUri) {
  try {
    ...
    // assert(['https:', 'http:'].includes(parsed.protocol));
    return parsed;
  } catch (err) {
    return invalidate('redirect_uris must only contain valid web uris');
  }
}
...
module.exports = function getSchema(provider) {
  ...
  class Schema {
    constructor(metadata) {
      ...
      Object.assign(this, DEFAULT, _.pick(metadata, RECOGNIZED_METADATA));
      ...
      if (this.subject_type === 'pairwise' && !this.sector_identifier_uri) {
        ...
        if (hosts.length === 1) {
          this.sector_identifier = hosts[0]; // eslint-disable-line prefer-destructuring
        } else {
