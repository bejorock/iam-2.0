...
module.exports = function getBaseToken(provider) {
  ...
  return class BaseToken {
    ...
    async save() {
      ...
      if(this.clientId) upsert.clientId = this.clientId
      if(this.accountId) upsert.username = this.accountId
      if(this.refId) upsert.refId = this.refId
    ...
    static async find(token = '', { ignoreExpiration = false } = {}) {
      ...
      try {
        ...
        if(stored.clientId) payload.clientId = stored.clientId
        if(stored.accountId) payload.username = stored.accountId
        if(stored.refId) payload.refId = stored.refId
