...
module.exports = function getClient(provider) {
  ...
  function add(metadata, dynamic) {
    const modifiedMeta = _.mapKeys(JSON.parse(JSON.stringify(metadata)), (value, key) => _.snakeCase(key))
    //console.log(modifiedMeta)
    //console.log(_.mapKeys(metadata, (value, key) => _.snakeCase(key)))
    //console.log(_.mapKeys(JSON.parse(JSON.stringify(metadata)), (value, key) => _.snakeCase(key)))
    return schemaValidate(new Client(), modifiedMeta) // eslint-disable-line no-use-before-define
      .then(sectorValidate)
      .then(buildKeyStore)
      .then((client) => {
  ...
  class Client {
    ...
    static async find(id, { fresh = false } = {}) {
      ...
      if (properties) return add(properties, false);
      return undefined;
    }
